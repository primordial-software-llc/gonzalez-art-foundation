...
namespace SlideshowCreator.Tests.Backpage
{
    class BackpageAdTableInfrastructure
    {
        ...
        public void Insert_Then_Get()
        {
            ...
            {
                {"source", new AttributeValue {S = BackpageAdAccess.SOURCE}},
                {"url", new AttributeValue {S = sample.Uri.AbsoluteUri}}
            };
            var refreshedConversion = client.GetItem(BackpageAdAccess.TABLE_NAME, key);
            var refreshed = new BackpageAdModelConversion().ConvertToPoco(refreshedConversion.Item);
